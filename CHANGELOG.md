...
## [Unreleased]
### Added
- `orch-tui` full-screen terminal control panel with keyboard/mouse navigation for launching, restoring, messaging, task updates, health checks, and session teardown through existing commands.
- `orch-restore` command to rebuild missing tmux panes from an existing `.agents/roster.json` after a tmux session is closed or killed.
- `--iterm-cc` / `-CC` presentation option for iTerm2 tmux control-mode attach.
- Roster resume metadata for restored agents, with automatic Claude session IDs and best-effort Codex session capture.
...
- `orch-preflight --repair` now preserves active roster entries when the entire tmux session is missing and points operators to `orch-restore`.
